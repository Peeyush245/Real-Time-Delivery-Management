    ...
    return (
      <div>
            ...
            <div>
              {deliveryExecutives.map((deliveryPerson) => (
                <div key={deliveryPerson.name}>
                  Delivery Executive {deliveryPerson.name} is at Location{' '}
                  {deliveryPerson.startPosition}
                </div>
              ))}
            </div>
        ...
        <Grid container style={{ marginTop: '40px' }}>
          <Grid xs={12}>
            <Button variant='contained' color='primary' onClick={submitOrders}>
              Submit
            </Button>
          </Grid>
        </Grid>
  ...
  const submitOrders = () => {
    ...
    for (let i = 0; i < deliveryExecutives.length; i++) {
      const shortestPath = findShortestPath(
        graph,
        deliveryExecutives[i].startPosition,
        orderData.pickupLocation
      );
      distanceToReachForEachExecutive.push(shortestPath.distance);
    }

    const selectedIndex = distanceToReachForEachExecutive.indexOf(
      Math.min(...distanceToReachForEachExecutive)
    );
    const selectedDeliveryExecutive = deliveryExecutives[selectedIndex];

    const pickupShortestPath = findShortestPath(
      graph,
      selectedDeliveryExecutive.startPosition,
      orderData.pickupLocation
    );
    const deliveryShortestPath = findShortestPath(
      graph,
      orderData.pickupLocation,
        ...
        <Table aria-label='simple table'>
          ...
          <TableBody>
            {dataList.map((item, index) => (
              <TableRow key={index} style ={ index % 2? { background : "#579686" }:{ background : "white" }}>
  ...
  return (
    <div>
      <>
        <Grid container>
          <Grid xs={12}>
            <h3>Real Time Delivery Management System</h3>
          </Grid>
        </Grid>
        <Grid container>
          <Grid item xs={12} md={6}>
            <div className='form'>{getDataTable()}</div>
          </Grid>
          <Grid item xs={12} md={6}>
            {finalForm()}
          </Grid>
            ...
            <Grid item xs={12}>
              <Grid container>
                <Grid item xs={6} md={6}>
                  <div>
                    <div>
                      {orderGraphData.deliveryExecutive} will go from{' '}
                      {orderGraphData.deliveryExecutiveStartingLocation} to{' '}
                      {orderData.pickupLocation} for Pickup
                    </div>
                    <Graph
                ...
                <Grid item xs={6} md={6}>
                  <div>
                    <div>
                      {orderGraphData.deliveryExecutive} will go from{' '}
                      {orderData.pickupLocation} to {orderData.dropLocation} for
                      Delivery
                    </div>
                    <Graph
